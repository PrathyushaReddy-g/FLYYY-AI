...
app = FastAPI(
    ...
    description="FLYYY.AI Privacy-Preserving Customer Data Platform - Protected by default, reveal by exception.",
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan
)
...
async def global_exception_handler(request: Request, exc: Exception):
    # Log sanitized message without plaintext PII
    return JSONResponse(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        content={"detail": "INTERNAL_SERVER_ERROR", "message": "An unexpected error occurred while processing the request."}
    )
